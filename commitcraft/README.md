...
The workflows, scripts, templates, and Conventional Commit policy are identical in both
runtimes — same 72-char subject cap, same `git add <file>` staging, same "never `--no-verify`",
same hard stops on hook failure, same derived default branch.
...
Hermes ships the *skill directory* and nothing else, so `scripts/` and `templates/` are
mirrored into it (generated by `hermes/sync-resources.sh`, CI-gated against drift). Scripts
resolve from `${HERMES_SKILL_DIR}/scripts/` and are invoked via `bash` — Hermes strips the
...
Then ask the agent to run CommitCraft (`commit`, `pr`, `setup`, `check`, …).

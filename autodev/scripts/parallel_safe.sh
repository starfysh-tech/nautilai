...
shopt -s nocasematch
if [[ "$text" =~ (migration|lockfile|package-lock|pnpm-lock|yarn\.lock|poetry\.lock|Cargo\.lock|schema|generated|codegen|build[[:space:]]config|vite\.config|webpack|tsconfig|settings\.py|dockerfile|compose) ]]; then
  echo false
else
  echo true
fi

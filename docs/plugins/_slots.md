...
| `{{INSTALL_CMD}}` | Fixed pattern | Always `plugin install <name>@nautilai`. Never hand-write a different form. **If the plugin is dual-runtime** (see [`../conventions/dual-runtime.md`](../conventions/dual-runtime.md)), the Install section carries **two** labelled `copyfield`s — a `.runtime-label` of "Claude Code" above the slash command, and one of "Hermes Agent" above `hermes skills install skills-sh/starfysh-tech/nautilai/<skill>` — plus an `#install .note` stating that no tap is needed. Install is where people copy from; a runtime's command must never be buried in Requirements. Single-runtime plugins keep one copyfield and no label. |
